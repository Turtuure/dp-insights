 ...
 *
 * Renders a full-width 2-column layout:
 *   LEFT  — title, body (the body fills all remaining height)
 *   RIGHT — excerpt (top), category, category_label, author, published_date,
 *           featured toggle, slug, action buttons (sticky bottom)
 *
 * Variables expected before include:
 * @var array  $insight        Pre-fill values, or [] for an empty form.
 *                              Keys: title, slug, category, category_label, author,
 *                              published_date, excerpt, content, featured.
 * @var string $primary_label  Submit button label ('Create' for new, 'Save' for edit).
 * @var bool   $show_delete    If true, render a Delete button (#if-delete) in the actions.
 */
...
/**
 * Split a stored DB datetime into ['date' => 'Y-m-d', 'time' => 'H:i']
 * so the form can render a separate date input + time trigger button.
 */
    ...
    <div class="insight-form__cols">
        ...
        <div class="insight-form__col insight-form__col--left">
            ...
            <div class="insight-form__field">
                <label class="insight-form__label" for="if-title">Title</label>
                <input type="text" id="if-title" name="title"
                       class="insight-form__input"
                       value="<?= $esc($v('title')) ?>" required>
            </div>

            <div class="insight-form__field insight-form__field--grow">
                <label class="insight-form__label" for="if-content">Body (HTML)</label>
                <textarea id="if-content" name="content"
                          class="insight-form__input insight-form__textarea insight-form__textarea--body"><?= $esc($v('content')) ?></textarea>
            </div>
        </div>
        ...
        <div class="insight-form__col insight-form__col--right">
            ...
            <div class="insight-form__col-scroll">
                ...
                <div class="insight-form__field">
                    <label class="insight-form__label" for="if-slug">
                        Slug
                        <span class="insight-form__hint">auto-generated from title</span>
                    </label>
                ...
                <div class="insight-form__field">
                    <label class="insight-form__label" for="if-excerpt">Excerpt</label>
                    <textarea id="if-excerpt" name="excerpt" rows="4"
                              class="insight-form__input insight-form__textarea"><?= $esc($v('excerpt')) ?></textarea>
                </div>
                ...
                <div class="insight-form__field">
                    ...
                    <label class="toggle-switch" for="if-featured">
                        <input type="checkbox" id="if-featured" name="featured"
                               <?= !empty($insight['featured']) ? 'checked' : '' ?>>
                        ...
                        <span class="toggle-switch__label" data-on="Highlighted on the public site" data-off="Hidden from the highlighted list">
                            Hidden from the highlighted list
                        </span>
                    </label>
                </div>
            </div>
            ...
            <div class="insight-form__actions">
                ...
                <?php endif; ?>
                <button type="button" class="btn btn--outline" id="if-save">Save</button>
